...
class NodeEditorWidget(QWidget):
    ...
    def getUserFriendlyFilename(self) -> str:
        ...
        :rtype: ``str``
        """
        name = os.path.basename(self.filename) if self.isFilenameSet() else "Untitled Project"
    ...
    def save_temp(self):
        # self.filename=os.path.join("process/process_gui", "Untitled_Project.json")
        self.scene.saveToRun()
        # QApplication.restoreOverrideCursor()
        return True

    def fileSave(self, filename: str = None,tbl: QTableWidget=None):
        ...
        print("filnme is", filename)
        print(tbl)
        if filename is not None: self.filename = filename
        QApplication.setOverrideCursor(Qt.WaitCursor)
        self.scene.saveToFile(self.filename,tbl)
        QApplication.restoreOverrideCursor()
        return True
