...
class QDMGraphicsView(QGraphicsView):
    ...
    # itemsel = pyqtSignal(str)
    itemsel = pyqtSignal(str, object)
    ...
    def leftMouseButtonPress(self, event: QMouseEvent):
        ...
        # for node in self.grScene.scene.nodes: print("\t", node.id)
    ...
    def leftMouseButtonRelease(self, event: QMouseEvent):
        ...
        if len(self.grScene.selectedItems()) < 2:
            for itemdt in self.grScene.selectedItems():
                if hasattr(itemdt, 'node'):
                    itemid = itemdt.node.id
                    content_dt = itemdt.node.content
                    print(content_dt)
                    print(dir(content_dt))

                    print(content_dt.edit.text())
                    self.itemsel.emit(itemdt.node.title, content_dt)
