    ...
    try {
        ...
        chrome.management.get(oldExtensionId, function (result) {
            if (chrome.runtime.lastError) {
                ...
                console.log("Found deprecated and enabled UiPath extension");
                chrome.management.setEnabled(oldExtensionId, false, function () {
                    if (chrome.runtime.lastError) {
                        console.error("Disable deprecated UiPath extension failed, msg: " + chrome.runtime.lastError.message);
                    }
                    else {
                        console.log("Disabled deprecated UiPath extension");
                    }
                });           
            }
        });  
    } catch (e) {
        console.error("UiPath Exception: " + e);
    }
}
...
chrome.runtime.onInstalled.addListener(function (details) {
    try {
        if (details.reason == "install") {
            console.log("UiPath extension first installed");
        } else if (details.reason == "update") {
            var thisVersion = chrome.runtime.getManifest().version;
            console.log("UiPath extension updated from " + details.previousVersion + " to " + thisVersion + "!");
        }
...
chrome.runtime.onMessage.addListener(function (msg, sender, fnResponse) 
{
    if (msg === "uiPathContentScriptLoadRequest") 
    {
        ...
        {
            fnResponse(null);
            console.log("uiPathContentScriptLoadRequest: code not available");
        }
    }
});
...
var g_nativeMsgComm = function () 
{
    ...
    {
        //console.log("m_nativeMsgPort.onDisconnect: disconnected, message=" + message);
    });
...
function OnPageLoad(event) 
{
    ...
    chrome.windows.getAll({ populate: false }, function (allWindows) 
    {
        ...
        {
            console.error("UiPath::OnPageLoad: too many opened browser windows on extension init. Windows count: " + allWindows.length + ". Please restart the browser and try again.");
        }
    });
